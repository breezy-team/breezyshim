...
fn create_test_graph() -> Graph {
    Python::with_gil(|py| {
        ...
        parents_dict.set_item(b"rev1".as_slice(), pyo3::types::PyTuple::empty(py)).unwrap();
        parents_dict.set_item(b"rev2".as_slice(), pyo3::types::PyTuple::new(py, &[pyo3::types::PyBytes::new(py, b"rev1")]).unwrap()).unwrap();
        parents_dict.set_item(b"rev3".as_slice(), pyo3::types::PyTuple::new(py, &[pyo3::types::PyBytes::new(py, b"rev2")]).unwrap()).unwrap();
        parents_dict.set_item(b"rev4".as_slice(), pyo3::types::PyTuple::new(py, &[pyo3::types::PyBytes::new(py, b"rev1")]).unwrap()).unwrap();
...
fn test_get_parent_map() {
    ...
    let parent_map = graph.get_parent_map(&[rev1.clone(), rev2.clone(), rev3.clone(), rev4.clone()]).unwrap();
